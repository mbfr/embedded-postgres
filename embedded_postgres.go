...
func (ep *EmbeddedPostgres) cleanDataDirectoryAndInit() error {
	...
	if err := ep.initDatabase(ep.config.postgresProcAttr, ep.config.binariesPath, ep.config.runtimePath, ep.config.dataPath, ep.config.username, ep.config.password, ep.config.locale, ep.syncedLogger.file); err != nil {
		return err
	}
...
func startPostgres(ep *EmbeddedPostgres) error {
	...
	postgresProcess.SysProcAttr = ep.config.postgresProcAttr
